class ResponsesController < ApplicationController
    ...
    def show_all_responses
      @survey = Survey.find(params[:survey_id])
      # @responses = Response.where(user_id: current_user.id, survey_id: params[:survey_id])
      @responses = current_user.responses
      @language = params[:language] || "English"
      if @responses.present?
        @questions = Question.where(survey_id: params[:survey_id], language: @language)
        @survey_results = SurveyResult.where(survey_id: params[:survey_id], users_id: current_user.id)
        @category = Category.where(id: @survey_results.pluck(:categories_id))
      else
        @questions = nil
        @survey_results = nil
        @category = nil
